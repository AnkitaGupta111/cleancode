	...
	public static Boolean isEqual(DtCondition condition, ConditionValue rhs, boolean ignoreCase) {
		...
		if (condition.getDataType() == StringConstants.DATATYPE_STRING) {
			...
			if (eCondition.getConditionValue().getStringConditionValue().equals("null")) {
				if (null == rhs.getStringConditionValue() && !rhs.getBooleanConditionValue()
						&& rhs.getDoubleConditionValue() == 0.0 && rhs.getLongConditionValue() == 0) {
					return true;
				}
				return false;
			}
			if (ignoreCase) {
				return eCondition.getConditionValue().getStringConditionValue()
						.equalsIgnoreCase(rhs.getStringConditionValue());
			}
		...
		if (condition.getDataType() == StringConstants.DATATYPE_STRING) {
			if (ignoreCase) {
				return Pattern.compile(Pattern.quote(eCondition.getConditionValue().getStringConditionValue()),
						Pattern.CASE_INSENSITIVE).matcher(rhs.getStringConditionValue()).find();
			}
			return Pattern.compile(Pattern.quote(eCondition.getConditionValue().getStringConditionValue()))
					.matcher(rhs.getStringConditionValue()).find();
		}
		return null;
	}
